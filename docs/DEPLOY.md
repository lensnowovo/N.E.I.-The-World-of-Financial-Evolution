...
`vercel.json` 配置每日 03:00 UTC 调用 `/api/cron/cleanup-rate-limits`，兼容 Vercel Hobby 的每日 Cron 限制，删除：

- `RateLimitBucket.expiresAt < now`（过期限流桶）；
- `ActivationCode.expiresAt < now − 7 天`（过期并超过保留期的激活码；仍有效/未过保留期的一律保留）。

部署 Memory Node 激活前，请在 Vercel Project Settings → Environment Variables 配置 `CRON_SECRET`。无需在 Cron Jobs 另填一遍；Vercel 会自动注入 Bearer Token。未配置时清理不会执行，且端点对任何调用返回 401。
